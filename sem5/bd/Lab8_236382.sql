...
begin
declare @startage int = 50, @endage int = 80;
IF @startage is null
  @startage=18;
select * from
(
select EmployeeID,FirstName,LastName,ROW_NUMBER() over (order by EmployeeID desc) as rownumber from Employees
where DATEDIFF(YEAR,BirthDate,GETDATE()) between @startage and @endage
)
as k1
where rownumber%3=0
end
