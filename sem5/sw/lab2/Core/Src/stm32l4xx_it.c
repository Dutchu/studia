...
#define IS_JOY_UP ((~(GPIOE->IDR) & JOY_PORT_UP) ? 1 : 0)
#define IS_JOY_DOWN ((~(GPIOE->IDR) & JOY_PORT_DOWN) ? 1 : 0)
...
void SysTick_Handler(void) {
	...
	if (refresh && IS_JOY_UP && !joy_pushed) {
	...
	if (refresh && IS_JOY_DOWN && !joy_pushed) {
	...
	if (refresh && IS_JOY_LEFT && !joy_pushed) {
	...
	if (refresh && IS_JOY_RIGHT && !joy_pushed) {
	...
	if (refresh && IS_JOY_PUSH && !joy_pushed) {
		leds_lvl[0U] = 0U;
		leds_lvl[1U] = 0U;
		leds_lvl[2U] = 0U;
	...
	if (!IS_JOY_RIGHT && !IS_JOY_LEFT && !IS_JOY_DOWN && !IS_JOY_UP) {
		joy_pushed = 0;
		was_joy_right = 0;
		was_joy_left = 0;
		was_joy_up = 0;
